...
def generate_project_requirements(
    ...
    """
    Memanggil Multi-AI Engine untuk menghasilkan draf dokumentasi kebutuhan software yang
    kaya dan mendalam: User Types (dengan Persona), Epics, User Stories, AC, Tech Notes,
    Test Cases, dan NFR.
    """
    if ai_rules:
    ...
    INSTRUKSI PENYUSUNAN LENGKAP:

    1. USER TYPES & PERSONAS
       - Identifikasi 4 hingga 6 tipe pengguna utama yang relevan.
       - Berikan deskripsi identitas dan peran masing-masing (maksimal 3 kalimat).
       - Setiap User Type WAJIB memiliki TEPAT 1 contoh persona fiktif yang realistis (nama, usia, lokasi, pekerjaan, about, goals, frustrations).

    2. EPICS
       - Susun 5 hingga 8 Epic komprehensif yang mencakup seluruh alur sistem (Autentikasi, Fitur Utama, Pengelolaan Data, Laporan/Monitoring, Pengaturan).
       - Deskripsi setiap Epic harus konkret (4-6 kalimat) menjelaskan tujuan bisnis, cakupan fitur, dan batasan.

    3. USER STORIES (INTI FITUR)
       - Setiap Epic wajib memiliki 2 hingga 4 User Story penting yang saling melengkapi.
       - Format description: 'Sebagai [user_type], saya ingin [aksi], agar [manfaat]' dengan main flow dan minimal 1 edge case.
       - Setiap story wajib memiliki 3-4 Acceptance Criteria format Given-When-Then terukur.
       - Setiap story wajib memiliki 2-3 Tech Notes actionable untuk developer.
       - Setiap story wajib memiliki 3-4 Test Cases konkret untuk tim QA.

    4. NON-FUNCTIONAL REQUIREMENTS (NFR)
       - Minimal 6 NFR kuantitatif (Performance, Security, Availability, Usability, Scalability, Compliance).

    Hasilkan output JSON valid yang mematuhi skema ProjectRequirementsOutput secara presisi.
    """

    # TAHAP 1: Coba jalankan dengan Gemini (Kapasitas & Pemahaman Arsitektur Terbaik)
    try:
        ...
        print(f"⚠️ Gemini gagal/limit ({gemini_err}), melakukan failover otomatis ke OpenRouter/Groq...")

        # TAHAP 2: Failover ke OpenRouter (LLaMA 3.3 70B / DeepSeek)
        try:
            chat_res = openrouter_client.chat.completions.create(
                model=OPENROUTER_MODEL,
                messages=[
                    {"role": "system", "content": "You are a professional system analyst. Output strictly valid JSON matching the ProjectRequirementsOutput schema without markdown wrapper."},
            ...
            print(f"⚠️ OpenRouter gagal ({openrouter_err}), beralih ke Groq Cloud...")

            # TAHAP 3: Failover ke Groq Cloud (Super Fast LPU)
            try:
                groq_res = groq_client.chat.completions.create(
                    model=GROQ_MODEL,
                    messages=[
                        {"role": "system", "content": "You are a professional system analyst. Output strictly valid JSON matching the ProjectRequirementsOutput schema."},
                        {"role": "user", "content": prompt_main}
                    ],
                    response_format={"type": "json_object"}
                )
                raw_text_groq = groq_res.choices[0].message.content
                return ProjectRequirementsOutput(**json.loads(raw_text_groq))
            except Exception as final_err:
                raise RuntimeError(f"Seluruh provider AI (Gemini, OpenRouter, Groq) gagal memproses: {str(final_err)}")
